  ...
  return (
    <Layout title="Figma Sync" description="Componentstatus Storybook vs Figma">
      <main style={{ paddingBlockStart: 'var(--rvo-space-xl)' }}>
        ...
        <p>Automatische vergelijking tussen Storybook-renders en Figma-exports. Visuele match is leidend voor de status (drempel 90%).</p>
              ...
              {data.rows.map((row) => (
                <tr key={row.name} className="rvo-table-row">
                  <td className="rvo-table-cell">
                    {row.reportHref
                      ? <a className="rvo-link" href={row.reportHref}>{row.name}</a>
                      : row.name}
                  </td>
                  <td className="rvo-table-cell">{row.lastChecked ?? '—'}</td>
                  <td className="rvo-table-cell">{row.matchPct ?? '—'}</td>
                  <td className="rvo-table-cell" style={{ color: row.passed === null ? undefined : row.passed ? 'var(--rvo-color-groen)' : 'var(--rvo-color-rood)', fontWeight: row.passed !== null ? 600 : undefined }}>
        ...
        <hr />
        <p><small>Figma-cijfers zijn een export van {data.exportDate}.</small></p>
        <p><strong>Instanties</strong> — huidig totaal gebruik van de component in alle Figma-bestanden van het team.</p>
        <p><strong>Inserts (jaar)</strong> — aantal keer ingevoegd in de afgelopen 12 maanden; maatstaf voor adoptie.</p>
        <p><strong>Detaches (jaar)</strong> — aantal keer losgekoppeld van de library; hoog = component voldoet niet aan de behoefte.</p>
        </div>
      </main>
    </Layout>
  );
