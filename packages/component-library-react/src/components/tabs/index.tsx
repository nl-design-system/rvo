...
export interface TabsProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'onChange'> {
  ...
  /**
   * Adds default padding to each tab panel.
   */
  panelPadding?: boolean;

  /**
   * Adds default border to each tab panel.
   */
  panelBorder?: boolean;

  /**
   * Optional child API via TabItem components.
   */
...
const isTabItemElement = (child: React.ReactNode): child is React.ReactElement<TabItemProps> =>
    ...
    if (children) {
      return React.Children.toArray(children)
        ...
        .map((child, index) => ({
          ...
          id: child.props.id ?? `${reactId}-${index}`,
        }));
    }

    return [];
  }, [items, tabs, children, reactId]);

  useEffect(() => {
    if (tabs && !items) {
      console.warn('[Tabs] The `tabs` prop is deprecated. Use `items` instead.');
    }
    if (!items && !tabs && !children) {
      console.warn('[Tabs] Missing tab data. Provide `items` (preferred), legacy `tabs`, or child `TabItem` nodes.');
    }
  }, [tabs, items, children]);
      ...
      {normalizedItems.map((item, index) => {
        ...
        return (
          <div
            ...
            className={clsx(
              'rvo-tabs__panel',
              !panelPadding && 'rvo-tabs__panel--no-padding',
              !panelBorder && 'rvo-tabs__panel--no-border',
            )}
            role="tabpanel"
